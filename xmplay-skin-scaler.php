<?php
	...
	/*
	XMPlay Skin Scaler 0.2 (2018-08-22) by Thomas Radeke
	...
	*/
	...
	$title = "XMPlay Skin Scaler 0.2 (2018-08-22) by Thomas Radeke";
  ...
  Example: php $self -i \"iXMPlay.xmpskin\" -s 2.0 -f triangle\n";
	...
	if($argc == 1){
		...
		if(is_numeric(strpos($i, "*"))){
			echo("Wildcard conversion. Looking for candidates...\n");
			$wildcard = glob(glob_escape($i));
			if(count($wildcard) > 0){
				foreach($wildcard as $match){
					if(isValidXmpskin($match)){
						echo("Found valid skin in '$match'\n");
						$inputs[] = $match;
					}
				}
			}
			...
			} elseif(!isValidXmpskin($i)){
				echo("Error: search for 'skinconfig.txt' failed on '$i'.\nPlease note that this tool cannot unpack old XMPlay skins in DLL format.\n");
			} else	{
				$inputs[] = $i;
			}
		}
